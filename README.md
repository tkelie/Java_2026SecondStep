...
#### 🌱 練習 D-1：ログを userId で groupBy：【実装済(2026/03/05)】

ログファイル（`userId LOGIN/LOGOUT`）を読み込み、  
Stream API を利用してユーザー単位のログを集計する。

**実装内容**

- `LogReader`
  - ファイルを読み込み `Optional<List<Log>>` を返す

- `LogCounter`
  - `groupingBy + counting` を利用して  
    ユーザーごとの `LOGIN` 回数を集計

- `UserFinder`
  - `groupingBy + counting + filter` を利用して  
    `LOGIN` が複数回あるユーザーを抽出
